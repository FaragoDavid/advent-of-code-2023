...
function part1(input) {
  return input.split`\n`.reduce((total, row) => {
    const nums = row.split``.filter((n) => !isNaN(Number(n)));

    return total + Number(`${nums[0]}${nums[nums.length - 1]}`);
  }, 0);
}

console.log("Part 1 expected:", 142, "actual: ", part1(example1));
console.log("Part 1 result:", part1(input)); //55017
...
const getNumber = (row, index) => {
  return NUM_WORDS.reduce((num, text, i) => {
    if (row.slice(index, index + text.length).join`` === text) return i + 1;
    return num;
  }, NaN);
};
...
function part2(input) {
  return input.split`\n`.reduce((total, row) => {
    const nums = row.split``.reduce((acc, char, index, currentRow) => {
      const num = getNumber(currentRow, index);
      if (!isNaN(Number(char)) || num) {
        acc.last = Number(char) || num;
        if (!acc.first) {
          acc.first = Number(char) || num;
        }
      }
      return acc;
    }, {});

    return total + Number(`${nums.first}${nums.last}`);
  }, 0);
}

console.log("Part 2 expected:", 281, "actual: ", part2(example2));
console.log("Part 2 result:", part2(input)); //53539
