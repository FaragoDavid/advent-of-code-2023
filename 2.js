...
function part1(input) {
  return input.split`\n`.reduce((total, row) => {
    const [gameId, game] = row.split`: `

    const sets = game.split`; `;
    let isGameValid = true;
    for (let setIndex = 0; setIndex < sets.length && isGameValid; setIndex++) {
      const records = sets[setIndex].split`, `;
      for (let recordIndex = 0; recordIndex < records.length && isGameValid; recordIndex++) {
        const [num, color] = records[recordIndex].split` `
        if(color === 'red' && Number(num) > 12 ) isGameValid = false;
        if(color === 'green' && Number(num) > 13 ) isGameValid = false;
        if(color === 'blue' && Number(num) > 14 ) isGameValid = false;
      }
    }

    return isGameValid ? total + Number(gameId.split` `[1]) : total
  }, 0)
}


console.log("Part 1 expected:", 8, "actual:", part1(example));
...
function part2(input) {
  return input.split`\n`.reduce((total, row) => {
    ...
    const tally = game.split`; `.reduce((tally, records) => {
      records.split`, `.forEach(record => {
        const [num, color] = record.split` `;
        if(color === 'red' && tally.red < Number(num)) tally.red = Number(num)
        else if(color === 'green' && tally.green < Number(num)) tally.green = Number(num)
        else if(color === 'blue' && tally.blue < Number(num)) tally.blue = Number(num)
      });
      return tally;
    }, {red:0, green: 0, blue: 0});

    return total += (tally.red * tally.green * tally.blue);
  }, 0)
}

console.log("Part 2 expected:", 2286 , "actual:", part2(example));
